        ...
        <Popup class="task__menu" ref="menuRef" @mousedown.stop >
          ...
          <template v-slot:content>
            <ul class="popup__action-list">
              ...
              <li class="popup__action">
                <button class="popup__action-button button button_type_popup button_color_black"
                  @click="toggleVisibilityTask(), closeMenu()"
                >
...
const toggleVisibilityTask = () => {
  tasksStore.toggleTaskVisibility(props.id)
}
const removeTask = () => {
  tasksStore.removeTask(props.id)
}
...
.task {
  ...
  border-radius: 8px;
  box-shadow: 0 0 2px  lighten(get-light($sectionBorderColor), 2%);
  background: get-light($bgColor, 'main');
